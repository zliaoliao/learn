...
这份 JS 高级刷题笔记内容丰富，已涵盖大多数面试高频点，逻辑清晰且细节到位，非常适合作为手撕代码准备材料。我已通读全文，以下是**校正、优化、补充建议**，分为三类：

---

## ✅ 校正与优化建议（针对已有内容）

### 1. `EventEmitter` 修复与增强

* `once` 中 `this.off(name, wrapper)` 应优先执行在调用 `fn(...params)` 前，以防函数抛错无法解绑。
* 增加边界校验如：`if (typeof fn !== 'function') throw new Error(...)` 更安全。
* `emit` 建议复制一份监听数组，避免监听函数内部的解绑行为影响当前循环。

### 2. `require` 实现中建议：

* 如果路径非 `.js`，可以默认加 `.js` 结尾，模拟 Node 默认行为。

  ```js
  if (!path.extname(absPath)) absPath += '.js';
  ```
* `module.exports` 初始值建议直接挂载在 `exports` 上：

  ```js
  const exports = module.exports = {};
  ```

### 3. `flat` 的 stack 解法建议用 `shift` 替换 `pop`，保持顺序，避免 `result.unshift`。

### 4. `reduce` 实现建议区分参数名 `prev, curr` 更易读。
...
class LRUCache {
  ...
  put(key, value) {
    if (this.cache.has(key)) this.cache.delete(key);
      ...
      this.cache.delete(this.cache.keys().next().value); // 删除最早的
    }
    this.cache.set(key, value);
  }
}
...
| ------------------ | -------------------- |
| 防抖                 | “清除定时器，重新开始”         |
| 节流                 | “有 timer 就别动手，没有才动手” |
| call               | “改 this，临时属性运行”      |
| bind               | “保存 this，等会再干”       |
| once               | “加壳运行，壳里解绑”          |
| promise.all        | “全成则成，有一败则败”         |
| promise.allSettled | “不管输赢，统一汇总”          |
